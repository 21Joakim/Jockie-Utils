...
public class ArgumentFactoryImpl implements IArgumentFactory {
	...
	public ArgumentFactoryImpl registerDiscordParsers() {
		...
		this.registerParser(Channel.class, (context, argument, value) -> {
			List<Channel> channels = Collections.unmodifiableList(ArgumentUtility.getTextChannelsByIdOrName(context.getMessage().getGuild(), value, true));
			if(channels.size() == 0) {
	...
	public ArgumentFactoryImpl unregisterDiscordParsers() {
		...
		this.unregisterParser(Channel.class);
